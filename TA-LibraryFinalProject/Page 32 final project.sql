...
(11038,'Henry and June: From "A Journal of Love"--The Unexpurgated Diary of AnaÃ¯s Nin (1931-1932)','Mariner Books'),
...
(762,'CrÃ³nica de una muerte anunciada','Vintage Espanol'),
...
(6,'J.K. Rowling'),
(11,'Douglas Adams'),
(320,'Gabriel GarcÃ­a MÃ¡rquez'),
(428,'Joan Didion'),
(706,'Jon Stewart'),
(762,'Gabriel GarcÃ­a MÃ¡rquez'),
(890,'John Steinbeck'),
(1078,'Pearl S. Buck'),
(1103,'Stephen King'),
...
AS
SELECT copies.No_of_copies, branch.BranchName FROM tbl_bookCopies copies
INNER JOIN tbl_books books ON books.BookId=copies.BookID
INNER JOIN tbl_libraryBranch branch ON copies.BranchID=branch.BranchID
WHERE books.Title='The Lost Tribe' and branch.BranchID=2;
GO

CREATE PROCEDURE query2
AS
SELECT SUM(No_of_copies) AS No_Copies, BranchName FROM tbl_bookCopies copies
INNER JOIN tbl_libraryBranch branch ON copies.BranchID=branch.BranchID
INNER JOIN tbl_books books ON books.BookId = copies.BookID
...
AS
SELECT books.Title,borrower.Name,borrower.BorrowerAddress FROM tbl_bookLoans loans
INNER JOIN tbl_borrower borrower ON loans.CardNo=borrower.CardNo
INNER JOIN tbl_books books ON BOOKS.BookId=loans.BookID
WHERE loans.DueDate = CONVERT (date, SYSDATETIME()) AND loans.BranchID=2;
GO

CREATE PROCEDURE query5
AS
UPDATE totalLoansByBranch
SET total_loans = (SELECT COUNT(BookID) FROM tbl_bookLoans WHERE BranchID=1)
WHERE branch_id=1;
UPDATE totalLoansByBranch
SET total_loans = (SELECT COUNT(BookID) FROM tbl_bookLoans WHERE BranchID=2)
WHERE branch_id=2;
UPDATE totalLoansByBranch
SET total_loans = (SELECT COUNT(BookID) FROM tbl_bookLoans WHERE BranchID=3)
WHERE branch_id=3;
UPDATE totalLoansByBranch
SET total_loans = (SELECT COUNT(BookID) FROM tbl_bookLoans WHERE BranchID=4)
WHERE branch_id=4;
UPDATE totalLoansByBranch
SET total_loans = (SELECT COUNT(BookID) FROM tbl_bookLoans WHERE BranchID=5)
WHERE branch_id=5;
UPDATE totalLoansByBranch
SET total_loans = (SELECT COUNT(BookID) FROM tbl_bookLoans WHERE BranchID=6)
WHERE branch_id=6;
UPDATE totalLoansByBranch
SET total_loans = (SELECT COUNT(BookID) FROM tbl_bookLoans WHERE BranchID=7)
WHERE branch_id=7;
UPDATE totalLoansByBranch
SET total_loans = (SELECT COUNT(BookID) FROM tbl_bookLoans WHERE BranchID=8)
WHERE branch_id=8;
SELECT branch.BranchName, total.total_loans FROM totalLoansByBranch total INNER JOIN tbl_libraryBranch branch ON branch.BranchID=total.branch_id
GO
